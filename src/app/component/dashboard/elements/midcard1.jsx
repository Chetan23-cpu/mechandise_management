"use client";
import styles from "../css/midcard1.module.css";
...
const MidCard1 = () => {
  ...
  // Load locations, divisions, and merchandises once (for the filter dropdowns)
  useEffect(() => {
    setLoadingOptions(true);
    Promise.all([
      fetch("/api/locations").then((res) => res.json()),
      fetch("/api/divisions").then((res) => res.json()),
      fetch("/api/merchandises?limit=1000").then((res) => res.json()),
    ])
      .then(([locationsData, divisionsData, merchandisesData]) => {
        setLocations(Array.isArray(locationsData) ? locationsData : []);
        setAllDivisions(Array.isArray(divisionsData) ? divisionsData : []);
        setAllMerchandises(
          Array.isArray(merchandisesData?.data) ? merchandisesData.data : [],
        );
      })
  ...
  const resolvedProductCode = (() => {
    ...
    const product = allMerchandises.find(
    ...
    const fetchTableData = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/dashboard-data/table?${params.toString()}`,
        );
        const json = await res.json();
  ...
  const availableProducts = allMerchandises.filter((m) => {
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute("download", `merchandise-movement-${dateSuffix}.csv`);

...
export async function POST(request) {
    try {
        ...
        const [id] = await db("print_pos").insert({
        ...
        const newPrintPos = await db("print_pos").where({ id }).first();

        const locationRow = await db("locations").where({ id: newPrintPos.location }).first();

...
export async function GET(request) {
  try {
    ...
    if (productCode) {
      query = query.whereRaw("LOWER(TRIM(dd.product_code)) = LOWER(TRIM(?))", [
        productCode,
      ]);
    }
    if (startDate) {
    ...
    return NextResponse.json(
      { error: "Failed to fetch print & pos dashboard table data" },
      { status: 500 },
    );
  }
}
